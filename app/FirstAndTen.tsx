...
export default function FirstAndTen() {
  ...
  const resetSetup = (phase: Phase = "deckMode") => { setSession(initial(phase)); setUndoStack([]); setSelected({ p1: "", p2: "" }); setSelectedVirtualCard(""); setOpeningReceiver(null); clearInputs(); };
  ...
  const startGame = (team: Team) => { setOpeningReceiver(null); act((s) => { if (s.gameMode === "virtual") Object.assign(s, dealVirtualDeck()); return { ...s, firstHalfOpener: team, offense: team, possessionNum: 1, ballPos: 20, down: 1, lineToGain: 30, phase: "play", turnStage: s.gameMode === "virtual" ? "offenseSelect" : null }; }); };
  ...
  return <main className="app">
    ...
    {(["play", "pat", "onsideChoice"] as Phase[]).includes(session.phase) && <>
      ...
      {session.driveOver ? <section className="card possession-over"><Eyebrow>Possession over</Eyebrow><h2>{session.driveOverMsg}</h2>{session.gameMode==="virtual"&&advanceLabel==="Start next drive"&&<div className="amber-banner">Hand the phone to <strong>{nameOf(nextDriveOffense)}</strong> — {nameOf(nextDriveOffense)} will be on offense for the next drive.</div>}{!session.overtime&&session.possessionNum===4&&<StatsSnapshot title="Halftime stats" p1Name={session.p1} p2Name={session.p2} stats={session.stats}/>}<button className="primary full" onClick={startNextDrive}>{advanceLabel}</button>{undoStack.length > 0 && <button className="undo-link" onClick={undo}>↶ Undo last play</button>}</section>
