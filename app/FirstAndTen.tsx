...
import {
  Card, CardColor, GameResult, MatchResult, MercyEvaluation, PlayerGameStats, PlayerProfile, RANKS, Rank, VirtualCard,
  ...
  dealVirtualDeck, drawOneVirtual, emptyPlayerGameStats, evaluateMathematicalMercy, filterGameResults, formatMercyExplanation, gamePlayerStats, isCpuGame, openingReceiverForChoice, playerStats, puntDistance, recordExtraPointResult, resolveMatch, shuffleCards, sortGameResults, valueOf, yd,
} from "./game-engine";
import {
...
type Session = {
  ...
  mercyRuleEnabled: boolean; mercyResult: MercyEvaluation | null; mercyExplanation: string; endReason: "regulation" | "overtime" | "mathematical_mercy" | null;
...
const initial = (phase: Phase = "home"): Session => ({
  ...
  mercyRuleEnabled: true, mercyResult: null, mercyExplanation: "", endReason: null,
});
...
export default function FirstAndTen() {
  ...
  const endDrive = (s: Session, msg: string, next: { offense: Team; ballPos: number }) => {
    ...
    const nextOffense = s.possessionNum === 4 ? other(s.firstHalfOpener) : next.offense;
    const mercy = evaluateMathematicalMercy({scores:s.scores,completedPossession:s.possessionNum,nextOffense,firstHalfOpener:s.firstHalfOpener,enabled:s.mercyRuleEnabled,overtime:s.overtime});
    if (mercy.shouldEnd) {
      s.mercyResult=mercy; s.mercyExplanation=formatMercyExplanation(mercy,{p1:s.p1,p2:s.p2},s.scores,s.possessionNum);
      s.endReason="mathematical_mercy"; s.phase="gameOver"; s.resultSaved=null; s.driveOver=false;
      addLog(s,`Game ended by mathematical mercy rule after possession ${s.possessionNum}. ${mercy.eliminationReason}`);
    }
  };
  ...
  const startGame = (team: Team) => { setOpeningReceiver(null); act((s) => { if (s.gameMode === "virtual") Object.assign(s, dealVirtualDeck()); return { ...s, firstHalfOpener: team, offense: team, possessionNum: 1, ballPos: 20, down: 1, lineToGain: 30, phase: "play", cpuSeed: s.cpuId ? Math.floor(Math.random() * 0xFFFFFFFF) : 0, cpuDecisionIndex: 0, humanPlayHistory: [], openingDrawPile: [], openingHumanCard: null, openingCpuCard: null, openingWinner: null, openingChoice: null, openingTieCount: 0, mercyResult: null, mercyExplanation: "", endReason: null, turnStage: s.gameMode === "virtual" ? "offenseSelect" : null }; }); };
  ...
  const startNextDrive = () => {
    act((s) => {
      ...
      if (!s.overtime && n >= 8) { s.phase = s.scores.p1 === s.scores.p2 ? "suddenDeathStart" : "gameOver"; if (s.phase === "gameOver") { s.resultSaved = null; s.endReason = "regulation"; } return s; }
      if (s.overtime && n % 2 === 0 && s.scores.p1 !== s.scores.p2) { s.phase = "gameOver"; s.resultSaved = null; s.endReason = "overtime"; return s; }
  ...
  const startOvertime = (team: Team) => act((s) => ({ ...s, overtime: true, otStarter: team, possessionNum: 9, offense: team, ballPos: 20, down: 1, lineToGain: 30, phase: "play", driveOver: false, mercyResult: null, mercyExplanation: "", endReason: null }));
  useEffect(() => {
  ...
  return <main className="app">
    ...
    {(["play", "pat", "onsideChoice"] as Phase[]).includes(session.phase) && <>
      <><Scoreboard s={session} quarter={quarter} /><Field s={session} />{showDownStatus&&<p className="down-distance-status" aria-live="polite">{downDistanceLabel(session.down,session.ballPos,session.lineToGain,session.twoPoint)}</p>}{!session.overtime&&(session.possessionNum===4||session.possessionNum===8)&&(session.phase==="pat"||session.twoPoint)&&<div className="amber-banner period-conversion-notice"><strong>{session.possessionNum===4?"The half":"Regulation"} ends after this conversion attempt.</strong> There is no kickoff or onside-kick attempt afterward.</div>}{session.gameMode==="virtual"&&<p className="deck-hud">{session.drawPile.length} cards left in the draw pile</p>}</>
    ...
    {session.phase === "gameOver" && <section className="card final"><Eyebrow>{session.endReason==="mathematical_mercy"?"Game ended by mathematical mercy rule":"Final"}</Eyebrow><h2 style={{color:accent(winnerTeam)}} tabIndex={-1}>{nameOf(winnerTeam)} wins!</h2><div className="final-score"><span>{session.p1}</span><strong>{session.scores.p1} – {session.scores.p2}</strong><span>{session.p2}</span></div>{session.mercyResult?.shouldEnd&&<MercyFinalSummary result={session.mercyResult} explanation={session.mercyExplanation} nameOf={nameOf}/>}<StatsSnapshot title="Final stats" p1Name={session.p1} p2Name={session.p2} stats={session.stats}/>{session.resultSaved===null&&undoStack.length>0&&<button className="undo-link final-undo" onClick={undo}>↶ Undo last play</button>}{guestMode ? <div className="save-panel"><p className="helper">Guest game complete. Profiles and results were not saved.</p><button className="primary" onClick={()=>resetSetup("deckMode")}>New game</button></div> : session.resultSaved === null ? <div className="save-panel"><p>Save this result to {nameOf(winnerTeam)}&apos;s and {nameOf(other(winnerTeam))}&apos;s records?</p><div className="action-row"><button className="primary" onClick={()=>saveDecision(true)}>Save result</button><button className="secondary" onClick={()=>saveDecision(false)}>Don&apos;t save</button></div></div> : <div className="save-panel"><p className={session.resultSaved ? "saved" : "helper"}>{session.resultSaved ? "✓ Saved to game history." : "Result not saved."}</p><div className="action-row"><button className="primary" onClick={()=>resetSetup("deckMode")}>New game</button><button className="secondary" onClick={()=>navigate("history")}>View history</button></div></div>}</section>}
...
function MercySetting({enabled,onChange}:{enabled:boolean;onChange:(enabled:boolean)=>void}){return <section className="mercy-setting" aria-labelledby="mercy-setting-title"><div><h3 id="mercy-setting-title">Mathematical Mercy Rule</h3><p>End the game only when the trailing team has no legal path to tie or take the lead.</p></div><div className="mercy-toggle" role="group" aria-label="Mathematical Mercy Rule"><button className={enabled?"selected":""} aria-pressed={enabled} onClick={()=>onChange(true)}>On</button><button className={!enabled?"selected":""} aria-pressed={!enabled} onClick={()=>onChange(false)}>Off</button></div></section>}
...
function MercyFinalSummary({result,explanation,nameOf}:{result:MercyEvaluation;explanation:string;nameOf:(team:Team)=>string}){return <section className="mercy-final" role="status" aria-live="assertive"><h3>Why the game ended early</h3><div className="mercy-summary-grid"><div><span>Deficit</span><strong>{result.deficit}</strong></div><div><span>Possessions left</span><strong>{result.remainingPossessions}</strong></div><div><span>Maximum comeback</span><strong>{result.maximumComeback}</strong></div><div><span>Next offense</span><strong>{result.nextOffense?nameOf(result.nextOffense):"—"}</strong></div></div><p>{explanation}</p></section>}
...
function RulesContent(){return <div className="rules-content"><ol className="rules-steps">
  ...
  <li><strong>Finish every score.</strong> A touchdown&apos;s extra-point or two-point attempt is always completed before halftime, the final, overtime, or an early mathematical ending. No kickoff or onside kick follows the last possession of a half.</li>
  ...
  <li><strong>Optional mercy rule.</strong> The pregame setting is on by default. At completed regulation possession boundaries, the app ends the game only when an exact check of every legal scoring, safety, possession, conversion, and onside-kick path proves the trailer cannot tie or lead. It never applies in overtime.</li>
...
function HistoryV2({players,games,onNew}:{players:PlayerProfile[];games:GameResult[];onNew:()=>void}){
  ...
  return <section className="card history">
    ...
    {recent.length===0?<p className="legacy-stats">No games match this filter.</p>:<div className="recent-games">{recent.map(game=>{const open=expanded===game.id,cpu=isCpuGame(game),mercy=game.endReason==="mathematical_mercy";return <article key={game.id} className={open?"expanded":""}><button className="game-summary" aria-expanded={open} onClick={()=>setExpanded(open?null:game.id)}><time>{gameDate(game.playedAt)}{game.overtime&&" (OT)"}</time><span><strong>{game.p1Name}</strong> - {game.p1Score} <strong>{game.p2Name}</strong> - {game.p2Score}{cpu&&<em className="cpu-game-tag">CPU - {game.cpuDifficulty||"Opponent"}</em>}{mercy&&<em className="mercy-game-tag">Mathematical Mercy{game.mercyAtPossession?` · ${game.mercyAtPossession} possessions`:""}</em>}</span><i aria-hidden="true">{open?"-":"+"}</i></button>{open&&<>{mercy&&game.mercyExplanation&&<section className="history-mercy"><strong>Why it ended early</strong><p>{game.mercyExplanation}</p></section>}{game.stats?<StatsSnapshot title="Game stats" p1Name={game.p1Name} p2Name={game.p2Name} stats={{p1:gamePlayerStats(game,"p1"),p2:gamePlayerStats(game,"p2")}}/>:<p className="legacy-stats">Detailed stats were not recorded for this earlier game.</p>}</>}</article>})}</div>}
  </section>;
}
